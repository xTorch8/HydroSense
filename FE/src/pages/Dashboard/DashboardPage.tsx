        ...
        const validateAndFetchData = async () => {
            ...
            try {
                ...
                const leaderboardResponse = await axios.get(
                  "https://api.hydrosense.nextora.my.id/dashboard/leaderboard",
                  { headers: { Authorization: `Bearer ${authContext.token}` } }
                );

                const historyResponse = await axios.get(
                    "https://api.hydrosense.nextora.my.id/dashboard/company/history",
                    { headers: { Authorization: `Bearer ${authContext.token}` } }
                );
      ...
      if (!companyProducts[companyId]) {
          try {
              const response = await axios.get(
                  `https://api.hydrosense.nextora.my.id/dashboard/company/${companyId}/products`,
                  { headers: { Authorization: `Bearer ${authContext?.token}` } }
              );
    ...
    return (
        <>
          <Navbar currentPage="Dashboard" />
            <h1 className="text-4xl font-bold text-center mt-8 underline">
                Leaderboard
            </h1>
                  ...
                  {leaderboard
                    ...
                    .map((company, index) => {
                      ...
                      : null;
                        ...
                        return (
                          <div key={company.company_id} className={`p-4 ${blockColor} cursor-pointer transition duration-300 ease-in-out hover:opacity-100`} onClick={() => toggleCompanyProducts(company.company_id)}>
                            <div className="flex items-center justify-between">
                              <div className="flex items-center space-x-4">
                                ...
                                {/* PFP Company */}
                                <img
                                  src={company.image_url || defaultImage}
                              ...
                              <button className={`transition-transform duration-300 ${
                                expandedCompany === company.company_id ? "rotate-180" : "rotate-0"
                              }`}>
                                ▼
                              </button>
                            </div>
                                ...
                                <div className="bg-darkblue text-white p-2 mt-2 rounded-lg">
                                  ...
                                  <strong>
                                    {companyProducts[company.company_id]
                                      ? (
                                        (company.clean_count / companyProducts[company.company_id].length) *
                                        100
                                      ).toFixed(2) + "%" + " Clean"
                                    : "0%"}
                                  </strong>
                            ...
                            {history.map((item) => (
                                <tr key={item.product_id} className="border-b text-center text-lg">
                                    <td className="text-center">
                                      <img
                                        src={item.result === "Clean" ? icons.correct : icons.wrong}
                                        alt={item.result}
